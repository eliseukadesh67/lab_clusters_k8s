...
Este guia descreve como retomar rapidamente o ambiente de Observabilidade (Prometheus/Grafana + microserviços gRPC) em Linux, utilizando os scripts bash do projeto. Inclui pré-requisitos, comandos de retomada, acessos rápidos, parada de port-forwards, redeploy manual, checagens e troubleshooting.
...
- `scripts/redeploy-grpc.sh`:
  - `docker build` e `docker push` de `eliseukadesh67/grpc-download-service:latest` e `eliseukadesh67/grpc-playlist-service:latest`.
